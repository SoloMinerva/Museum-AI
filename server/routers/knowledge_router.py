...
class KnowledgeQueryBody(BaseModel):
    """知识库查询体：兼容 {query, meta} 与 {query_text, top_k}（curl 示例）。"""
...
async def query_knowledge_base(
    ...
    """查询知识库（Body 可为 {query, meta} 或 {query_text, top_k}）"""
    q = body.resolved_query()
    if not q:
        raise HTTPException(status_code=400, detail="query 与 query_text 至少填一个")
    meta = body.resolved_meta()
    logger.debug(f"Query knowledge base {db_id}: {q} {meta=}")
    try:
        result = await knowledge_base.aquery(q, db_id=db_id, **meta)
...
async def query_test(
    ...
    """测试查询知识库（Body 同 /query）"""
    q = body.resolved_query()
    if not q:
        raise HTTPException(status_code=400, detail="query 与 query_text 至少填一个")
    meta = body.resolved_meta()
    logger.debug(f"Query test in {db_id}: {q} {meta=}")
    try:
        result = await knowledge_base.aquery(q, db_id=db_id, **meta)
        return result
    except Exception as e:
