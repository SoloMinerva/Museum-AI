...
class ChromaKB(KnowledgeBase):
    ...
    def _split_text_into_chunks(self, text: str, file_id: str, filename: str, params: dict) -> list[dict]:
        """将文本分割成块"""
        # 检查是否使用分隔符分割模式
        use_delimiter_split = params.get("use_delimiter_split", False)
        # 检查是否使用QA分割模式
        use_qa_split = params.get("use_qa_split", False)

        if use_delimiter_split:
            # 使用分隔符分割模式（如按【】拆分文物条目）
            delimiter = params.get("delimiter", "【")
            chunks = split_text_by_delimiter(text, file_id, filename, delimiter, params)
        elif use_qa_split:
            # 使用QA分割模式
        ...
        for item in items:
            ...
            try:
                ...
                if chunks:
                    ...
                    # 插入到 ChromaDB - 分批处理以避免超出 API 批次大小限制
                    batch_size = 16  # 减小批次大小以避免 API 限流
    ...
    async def add_image_embeddings(self, db_id: str, item: str, params: dict | None):
        ...
        try:
            ...
            logger.warning("跳过图片向量入库（可选依赖缺失: {}）: {}", e.name, item)
        except Exception as e:
            # logger.error(f"处理{content_type} {item} 失败: {e}, {traceback.format_exc()}")
            # self.files_meta[file_id]["status"] = "failed"
            # self._save_metadata()
