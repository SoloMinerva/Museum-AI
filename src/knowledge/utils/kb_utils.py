...
def split_text_by_delimiter(
    text: str, file_id: str, filename: str, delimiter: str = "【", params: dict = {}
) -> list[dict]:
    """
    按分隔符将文本拆分成块。每个分隔符标记一条独立记录的开头。

    例如文物数据以 【文物名】 开头，则用 delimiter="【" 切分，
    每条文物成为一个独立 chunk。
    """
    import re
    ...
    logger.debug(f"Successfully split text into {len(chunks)} chunks using delimiter '{delimiter}'")
    return chunks
