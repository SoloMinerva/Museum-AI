...
class MuseumArtifact(Base):
    """博物馆爬虫数据缓存（SQLite），便于联调检索与知识库导入。"""

    __tablename__ = "museum_artifacts"
    __table_args__ = (
        UniqueConstraint("source", "external_id", name="uq_museum_artifacts_source_external_id"),
    )

    id = Column(Integer, primary_key=True, autoincrement=True, comment="Primary key")
    source = Column(String(32), nullable=False, index=True, comment="爬虫标识，如 hbkgy、sxhm")
    external_id = Column(String(64), nullable=False, index=True, comment="来源站藏品 id")
    museum = Column(String(256), nullable=True, comment="收藏单位")
    name = Column(String(512), nullable=True, comment="文物名称")
    detail_url = Column(String(2048), nullable=True, comment="详情页 URL")
    image_url = Column(String(2048), nullable=True, comment="图片 URL")
    description = Column(Text, nullable=True, comment="知识库用描述（与 *_kb_import 一致）")
    payload = Column(JSON, nullable=True, comment="抓取原始字段（SQLite 存为 TEXT JSON）")
    created_at = Column(DateTime, default=utc_now, comment="写入时间")
